...
namespace TerraformTool
{
    public class InGameTerrainTool : ToolBase
    {
            ...
            if (this.m_mouseRayValid && this.enabled && this.m_mode != InGameTerrainTool.Mode.ResourceSand)
            {
                ...
                for (int i = minX - 16; i <= maxX + 16; i += 16)
                {
                    for (int j = minZ - 16; j <= maxZ + 16; j += 16)
                    {
                        ...
                        {
                            float dx = i - this.m_mousePosition.x;
                            float dz = j - this.m_mousePosition.z;
        ...
        private void GetBrushBounds(out int minX, out int minZ, out int maxX, out int maxZ, bool screenPos = false)
        {
            int res = 1080;
        ...
        private void ApplyBrush()
        {
            ...
            int maxX;
            int maxZ;
            this.GetBrushBounds(out minX, out minZ, out maxX, out maxZ);
            int originalHeight;
            ushort endHeight = 0;
            ...
            for (int i = minZ; i <= maxZ; i++)
            {
                for (int j = minX; j <= maxX; j++)
                {
                    if (this.m_mode == InGameTerrainTool.Mode.Point)
                    {
                        ...
                        int diff = this.m_trenchDepth * 64 * elevationStep * (this.m_mouseLeftDown ? 1 : -1);
                        originalHeight = (int)this.m_backupHeights[i * 1081 + j];
                    ...
                    else
                    {
                        ...
                        {
                            targetHeight = originalHeight + this.m_trenchDepth * 64 * (this.m_mouseLeftDown ? 1 : -1);
                        }
                        else if (this.m_mode == InGameTerrainTool.Mode.Level)
                        {
                            targetHeight = this.m_startPosition.y * 64;
                        }
                        ...
                        else if (this.m_mode == InGameTerrainTool.Mode.Slope)
                        {
                            ...
                            targetHeight = Mathf.Lerp(this.m_startPosition.y, this.m_endPosition.y, t2) * 64;
                        }
                    ...
                    {
                        this.m_rawHeights[i * 1081 + j] = endHeight;
